...
Our first aim is to write some code that tells our program which folder in our Home directory to open and read. The 'os' module has a function called 'os.getcwd()'. Have a look in the Python documentation for this function. What does it do? 

We can see from the documentation that os.getcwd() will 'return a string representing the current working directory'. If we type it in a cell under our import command, it should return our Home directory (unless you have set Jupyter Notebooks to have a different default working directory):

```python
os.getcwd()
```

If you remember the concept of variables from previous lessons, you may know that you can save the output of a function like os.getcwd() as a variable. When we do this, we give our code an easy address to our working directory, which we can invoke by just using the variable. This is useful if you want to always be sure what directory you are in, as this will ensure the output always prints your working directory. Let's do this now in a new cell:

```python
currentworkingdirectory = os.getcwd()
print(currentworkingdirectory)
```
In this code, we assign os.getcwd() the name 'currentworkingdirectory'. This is now a variable. When we print it, we should see our Home directory. This links the function of finding the working directory (which is just os.getcwd()) with the name 'currentworkingdirectory'. Now, any time we want the code to find the working directory, we just use this variable name. 
...
print(currentworkingdirectory)
```
But all this does is name the place in our computer where we already are. We want to tell the computer to move to the folder where we placed our new documents. The 'os' module has another function which can help us: os.path.join(). If we look at the documentation, it tells us that this function will 'join one or more path segments intelligently'. Let us join the paths of our Home Directory and our desired folder:
...
**Note:** You may have noticed in the documentation that there is another function called os.chdir(), which will 'change the current working directory to path.' Why not use this? Well, with os.chdir(), you fully move the entire program into the new directory, and lose the information about where you moved from. os.path.join() preserves the entire path you want to travel (from Home directory to specific folder), and this is easier to follow in more complex programs. Both do what we want, but many programmers prefer os.path.join(), so we will stick with it for now!
...
with os.scandir(targetdirectory) as currentfiles:
    for file in currentfiles:
        print(file.name)
```
Notice the first line here, with the 'with...as' statement. This is a way to create a code object, which we call 'currentfiles', that is scanned by os.scandir(). It knows to look in 'targetdirectory' since we put it as an argument in the curved brackets of os.scandir. 

Then we create the first level of the loop with our colon (':') after the end of the first line. This tells Python to open targetdirectory, scan the currentfiles, and then do something. Everything after the colon and indented is part of this task.

The next level of our loop, 'for file in currentfiles:' tells Python to iterate over every file in our folder, 'targetdirectory'.
Finally, the last line tells Python to print the file name for us every time it iterates through. Run this code, and if you get a list of the files we named earlier, then the program is working! Note that it will print the filesnames in a random order unless we sort them, which we won't bother with for now. 

There is one more issue to deal with before we can write our functions. Some operating systems, such as Mac or Linux, often have hidden files as a normal part of the system. If you saw '.DS_Store' when you ran the code above, then you have seen a hidden file. When writing code, we often have to exclude such files from being read by our code.There are multiple ways to do this, but since we only have two text (.txt) files in our folder, we can simply tell Python to only deal with files that have names ending in .txt:
...
Next, we want to open up each file in the directory, and print its contents (if any). Our files are currently empty, but we can still run the same code as if they had contents. Run the following code now in a new cell:
...
```
We have added two new lines to the end of our current program. Let's look at them one-by-one:
...
Inside the parentheses are the specific instructions, or arguments, for the 'open' command. The first is file.path, which is the like the 'house number' of each file in the directory. We also have 'r'. This is known as a file mode, and it tells Python to open the file in order to 'read' it only, and not do anything else. 

Notice that we call the whole thing 'currentfile'. We looped through all files as currentfiles (plural), and now are going to each one as currentfile (singular). This is stylistic. You could use any word for these variables. It's best to use words that make good sense to humans who may read your code later! Finally, as we are beginning a loop, we end the line with a colon (:).
...
It is simple to make a function. We simply write a line of code in the following format: def <function name> ():'. Here it is on our code:
...
write_content()
```
1. Line 1 of this function defines its name, write_content, and specifies that anything after it which is indented is a part of this function. 
2. Next, we have our os.scandir command, which is still reading our target directory and the current files in it.
3.  On lines 3 and 4 we have our for loop, which tells Python to iterate through every file in our target directory (named 'currentfiles'). If the file is a text (.txt) file, Python is instructed to open it.
4.  The next level of the loop, lines 5-6, is a bit different from our previous function. Notice that 'r' now is 'r+'. This tells Python to open the file in such a way that it both reads **and** modifies the file.
5.  To write something into our files, we put it in quotes inside the curved brackets after our .write command.

At the end of the function, we 'call' it by 'write_content()'. This tells Python to run the function we just described. 
**Note that this loop will insert the same phrase into every file in our directory.**

Run this code and then check the output by opening up your files. Is the sentence in each of them? If not, check that you have the correct level of indentation for every line; this is one of the most common errors in Python.

### Function 3: Getting a Description of File Contents
...
## The Entire Picture
Now that we have our three functions, let's look at the entire program we wrote:
...
<li>It is 'modular'. That means each function stands on its own.</li>
...
If you are noticing these things, congratulations! You are developing an eye for good code. High-quality Python functions should be as independent as possible. Ideally, each should do a single, specific task, and we want to minimise any one function being dependent upon another (though sometimes this is unavoidable). Python code should be easy to read for someone who hasn't written it. This means using descriptive and sensible variable names. 
...
```python
# this is a template, don't run this!
print ('Message to self', printedthing)
```
So, according to that template, for the very first thing we print, the current working directory, we could write:
